...
<body class="flex flex-col min-h-screen" style="background:#f0f6fc;">
    ...
    <header id="topbar"
        ...
        <div class="flex items-center gap-3">
            <a href="{{ route('annuaire.index') }}" class="flex items-center gap-3 no-underline" aria-label="Accueil">
                ...
                <div>
                    ...
                    <div class="uppercase tracking-widest" style="font-size:10px;color:#7aaecc;">Annuaire numérique des agents</div>
                </div>
            </a>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2 text-sm" style="color:#4a7fa0;">
                <div class="flex items-center justify-center rounded-full font-semibold text-xs text-white flex-shrink-0"
            ...
            <form method="POST" action="{{ route('logout') }}">
                ...
                <button type="submit"
                        ...
                        style="color:#7aaecc;border:0.5px solid #d0e8f8;background:#fff;"
    ...
    <div class="flex" style="margin-top:63px; min-height:calc(100vh - 63px);">
        ...
        <aside class="sidebar-scroll flex flex-col flex-shrink-0 overflow-y-auto"
            ...
            {{-- Statistiques --}}
            <div class="p-3 border-b" style="border-color:#e0eff8;">
            ...
            <nav class="flex-1 p-2">
                ...
                <a href="{{ route('annuaire.index') }}"
                   class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm transition mb-0.5
                          {{ request()->routeIs('annuaire.index') && !request()->entity_id ? 'font-medium' : '' }}"
                   style="{{ request()->routeIs('annuaire.index') && !request()->entity_id
                             ? 'background:#e8f4fd;color:#0d4f7c;'
                             : 'color:#3a6a8a;' }}">
                ...
                @foreach(\App\Models\Entity::roots()->with('childrenRecursive')->get() as $direction)
                    <div x-data="{ open: false }">
                        <button @click="open = !open"
                                class="w-full flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm transition mb-0.5 text-left"
                                style="color:#3a6a8a;"
                                onmouseover="this.style.background='#e8f4fd';this.style.color='#0d4f7c';"
                                onmouseout="this.style.background='';this.style.color='#3a6a8a';">
                            <div class="w-2 h-2 rounded-full flex-shrink-0" style="background:#1a7fc1;"></div>
                            <span class="flex-1 truncate">{{ $direction->nom }}</span>
                            <svg class="w-3 h-3 fill-current transition-transform" :class="open ? 'rotate-180' : ''" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z"/></svg>
                        </button>
                        <div x-show="open" x-cloak>
                            @foreach($direction->children as $child)
                                <a href="{{ route('annuaire.index', ['entity_id' => $child->id]) }}"
                                   class="flex items-center gap-2 px-2 py-1 rounded-lg text-xs transition mb-0.5 ml-4"
                                   style="color:#5a8aa8;"
                ...
                {{-- Administration --}}
                @if(auth()->user()->role === 'admin')
                    <div class="text-xs font-semibold uppercase tracking-widest px-2 py-2 mt-2" style="color:#7aaecc;">Administration</div>
                    <a href="{{ route('admin.entities') }}"
                       class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm transition mb-0.5"
                       style="{{ request()->routeIs('admin.entities') ? 'background:#e8f4fd;color:#0d4f7c;' : 'color:#3a6a8a;' }}"
                    ...
                    <a href="{{ route('admin.fonctions') }}"
                       class="flex items-center gap-2 px-2 py-1.5 rounded-lg text-sm transition mb-0.5"
                       style="{{ request()->routeIs('admin.fonctions') ? 'background:#e8f4fd;color:#0d4f7c;' : 'color:#3a6a8a;' }}"
                       onmouseover="this.style.background='#e8f4fd';" onmouseout="this.style.background='';">
        ...
        <main class="flex-1 min-w-0">
            {{-- Flash messages --}}
            @if(session('success'))
                <div class="mx-5 mt-4 flex items-center gap-3 px-4 py-3 rounded-xl text-sm"
                     style="background:#f0fdf4;border:0.5px solid #bbf7d0;color:#166534;"
                     x-data="{ show: true }" x-show="show" x-init="setTimeout(() => show = false, 3000)">
                    <div class="w-2 h-2 rounded-full flex-shrink-0" style="background:#22c55e;"></div>
                    {{ session('success') }}
                </div>
            @endif
            @if(session('error'))
